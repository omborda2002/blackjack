...
class BlackjackEnv:
    def __init__(self, use_counting=False, dealer_hits_soft_17=False, blackjack_payout=(3, 2),
                 use_true_count=False, decks=1, use_bet_scaling=False, eps_floor=0.05,
                 reward_shaping=False, toughest=False, double_after_split=False):
        ...
        # Apply toughest rules if needed
        if toughest:
    ...
    def _dealer_play(self):
        # Dealer's play logic according to rules
        while True:
            value, usable_ace = self._hand_value(self.dealer)
            if value < 17:
                self.dealer.append(self._draw_card())
            elif value == 17 and self.dealer_hits_soft_17 and usable_ace:
                self.dealer.append(self._draw_card())  # Soft 17 case
            else:
                break

    def _can_double_after_split(self):
        return self.double_after_split

    def apply_toughest_rules(self):
        # Set environment to most challenging configuration
    ...
    def shuffle_deck(self):
        # Shuffle deck and reset counters
    ...
    def draw_card(self):
        # Auto-shuffle at 75% penetration
    ...
    def hand_value(self, hand):
        # Compute total value considering Aces
